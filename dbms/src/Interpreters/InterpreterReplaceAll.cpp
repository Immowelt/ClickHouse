...
{
    severity_column = ColumnString::create();
    message_column = ColumnString::create();
    parameter_column = ColumnString::create();
}

void InterpreterReplaceAll::log(String severity, String message, String parameter)
{
    severity_column->insert(severity);
    message_column->insert(message);
    parameter_column->insert(parameter);
...
bool InterpreterReplaceAll::streamContainsOldValue(std::shared_ptr<MergeTreeBlockInputStream> stream)
{
    while(true)
    {
        ...
        ColumnString * col = dynamic_cast<ColumnString *>(oldcolumn.column->assumeMutable().get());
    ...
    if(!merge_tree)
    {
        ...
        if (!repl_merge_tree)
        {
            ...
            res.in =  std::make_shared<OneBlockInputStream>(Block
                    {
                {
                std::move(severity_column),
                std::make_shared<DataTypeString>(),
                "severity"
                },
                {
                std::move(message_column),
                std::make_shared<DataTypeString>(),
                "message"
                },
                {
                std::move(parameter_column),
                std::make_shared<DataTypeString>(),
                "parameter"
                }
...
BlockIO InterpreterReplaceAll::singleColumnReplace(MergeTreeStreamPartMap& parts, StorageMergeTree * merge_tree, StorageReplicatedMergeTree * repl_merge_tree)
{
    ...
    for (auto part_pair : parts)
    {
        ...
        if (b.rows() > 0)
        {
            ...
            auto column(b.getColumnsWithTypeAndName()[0]);
            if (merge_tree)
            {
...
Block InterpreterReplaceAll::readAndReplace(std::shared_ptr<MergeTreeBlockInputStream> readStream, size_t & replaced)
{
    ...
    size_t pos = b.getPositionByName(column);
    ColumnWithTypeAndName & oldcolumn = b.getByPosition(pos);
    auto newcolumn = ColumnString::create();
    const ColumnString * col = dynamic_cast<const ColumnString *>(oldcolumn.column.get());
    ...
    for(size_t row = 0; row < b.rows(); ++row)
    {
        ...
        if(!strcasecmp(s.data, oldvalue.c_str()))
        {
            newcolumn->insertDataWithTerminatingZero(newval, newvallen);
            ++replaced;
        }
        else
        {
            newcolumn->insertFrom(*col, row);
        }
    }
